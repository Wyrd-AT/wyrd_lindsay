...
export type Summary = 'Normal' | 'Ausente' | 'Alarmado' | 'Reconhecido' | 'Alarme OFF' | 'Desconhecido';
export type Monitor = { statusSw1: string; statusSw2: string; armadilha: number; statusTensao: Summary };
export type ParsedSW = { status_manutencao: '0' | '1'; painel_1: string; painel_2?: string; monitores: Monitor[] };
export type DeviceCard = { id: string; title: string; statuses: { label: string; value: string | number }[] };
    ...
    if (data.monitores &&  typeof data.monitores === 'object') {
        ...
        for (const k of keys) {
            ...
            mArr.push({
                ...
                armadilha:  typeof m.armadilha === 'number' ? m.armadilha : Number(m.armadilha || 0),
...
export function monitoresToVoltageMap(doc?: RecentTensaoDoc): Map<number, number> {
