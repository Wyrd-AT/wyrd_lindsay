...
const useTorresStore = () => {
  ...
  const torres = useMemo(() => {
    ...
    const sortedMessages = processed.sort((a, b) => new Date(b.date) - new Date(a.date));

    // Retorna a mensagem mais recente, se houver alguma
    return sortedMessages.length > 0 ? sortedMessages[0] : null;
  }, [parsedMessages, isLoading, error]);

  return { torres, isLoading, error };
};

export default useTorresStore;
