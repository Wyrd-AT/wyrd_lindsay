...
def export_to_csv(irrigadores: List[Dict], output_file: str):
    ...
    priority_fields = ['_id', '_rev', 'codigo', 'nome', 'name', 'table', 'type', 
                       'owner_id', 'gerente_id', 'companyId', 'ativo', 
...
def print_summary(irrigadores: List[Dict]):
    ...
    for i, irrigador in enumerate(irrigadores[:5], 1):
        print(f"  {i}. {Colors.BOLD}{irrigador.get('codigo', 'Sem código')}{Colors.ENDC}")
        print(f"     ID: {irrigador.get('_id', 'N/A')}")
        print(f"     Nome: {irrigador.get('nome') or irrigador.get('name') or 'N/A'}")
        print(f"     Owner: {irrigador.get('owner_id') or irrigador.get('companyId') or 'N/A'}")
        print()
...
Exemplos:
  ...
  python scripts/export_irrigadores.py --output irrigadores.json --csv irrigadores.csv
    ...
    try:
        ...
        print(f"{Colors.OKGREEN}✅ Encontrados {len(irrigadores)} irrigador(es){Colors.ENDC}\n")
    except Exception as e:
        print(f"{Colors.FAIL}❌ Erro ao buscar irrigadores: {e}{Colors.ENDC}")
        sys.exit(1)
    ...
    if args.output:
        ...
        print(f"{Colors.WARNING}💡 Use --pretty para ver JSON formatado no console{Colors.ENDC}")
