...
def get_users_from_db(db) -> Dict[str, Dict]:
    ...
    """
    users = {"admins": {}, "revendas": {}, "clientes": {}}
...
def infer_owner_id(
    pivo: Dict, users: Dict[str, Dict], default_owner: Optional[str] = None
) -> tuple:
    """
...
def update_pivo_fields(
    ...
    if not updates:
        return {"success": True, "action": "sem mudanças", "pivo_id": pivo_id}
    ...
    try:
        ...
        return {"success": False, "action": "erro", "pivo_id": pivo_id, "error": str(e)}
...
Exemplos:
  ...
  python scripts/update_pivos_fields.py --default-owner admin@company.com
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Apenas simular, não salvar no CouchDB"
    )
    ...
    print(
        f"{Colors.OKCYAN}Buscando usuários cadastrados (admin, revenda, cliente)...{Colors.ENDC}\n"
    )
    ...
    if total_users == 0:
        ...
        print(
            f"{Colors.WARNING}   Os pivôs serão atribuídos ao default-owner se fornecido{Colors.ENDC}\n"
        )
    ...
    try:
        ...
        pivos = list(result)
        print(
            f"{Colors.OKGREEN}✅ Encontrados {len(pivos)} irrigador(es){Colors.ENDC}\n"
        )
    except Exception as e:
    ...
    # Atualizar cada pivô
    print(
        f"{Colors.OKCYAN}Atualizando campos usando dados dos usuários cadastrados...{Colors.ENDC}\n"
    )

    results = {"updated": [], "no_changes": [], "errors": []}
    ...
    for pivo in pivos:
        ...
        if result["success"]:
            if result["action"] == "atualizado":
                ...
                print(
                    f"{Colors.OKGREEN}✅ {result['pivo_id']}: {updates_str}{Colors.ENDC}"
                )
                ...
                updates_str = ", ".join(f"{k}={v}" for k, v in updates.items())
                print(
                    f"{Colors.OKCYAN}🔍 {result['pivo_id']}: {updates_str} (simulado){Colors.ENDC}"
                )
            else:
                results["no_changes"].append(result)
                print(
                    f"{Colors.OKBLUE}ℹ️  {result['pivo_id']}: sem mudanças necessárias{Colors.ENDC}"
                )
        else:
            results["errors"].append(result)
            print(
                f"{Colors.FAIL}❌ {result['pivo_id']}: {result.get('error', 'erro desconhecido')}{Colors.ENDC}"
            )
    ...
    if args.dry_run:
        ...
        print(
            f"{Colors.WARNING}Execute sem --dry-run para aplicar as mudanças{Colors.ENDC}"
        )
