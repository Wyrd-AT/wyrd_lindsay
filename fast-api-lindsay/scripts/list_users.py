...
def get_all_users(db, user_type: Optional[str] = None, status_filter: Optional[str] = None) -> Dict[str, List[Dict]]:
    """
    ...
    try:
        ...
        if sum(len(v) for v in users.values()) == 0:
            print(f"{Colors.WARNING}⚠️  Query Mango não retornou resultados, tentando _all_docs...{Colors.ENDC}")
            ...
            for row in all_docs:
                ...
                if status_filter and doc.get("status", "").lower() != status_filter.lower():
                    continue
...
def print_admins(admins: List[Dict]):
    ...
    for i, admin in enumerate(admins, 1):
        ...
        print(f"   {Colors.OKCYAN}Status:{Colors.ENDC} {status_color}{status}{Colors.ENDC}")
...
def print_revendas(revendas: List[Dict]):
    ...
    print(f"{Colors.BOLD}{Colors.OKBLUE}Total: {len(revendas)} revenda(s){Colors.ENDC}\n")
...
def print_clientes(clientes: List[Dict]):
    ...
    print(f"{Colors.BOLD}{Colors.OKBLUE}Total: {len(clientes)} cliente(s){Colors.ENDC}\n")
...
Exemplos:
  ...
  python scripts/list_users.py --status active    # Apenas ativos
    ...
    if total == 0:
        print(f"{Colors.WARNING}⚠️  Nenhum usuário encontrado com os filtros especificados.{Colors.ENDC}")
        sys.exit(0)
