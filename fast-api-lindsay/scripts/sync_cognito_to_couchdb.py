...
def get_cognito_users(
    cognito_client, user_pool_id: str, user_type_filter: Optional[str] = None
) -> List[Dict]:
    """
    ...
    try:
        while True:
            ...
            for user in response.get("Users", []):
                ...
                user_data = {
                    ...
                    "created_at": user.get("UserCreateDate", datetime.now()).isoformat()
                    if hasattr(user.get("UserCreateDate", datetime.now()), "isoformat")
                    else str(user.get("UserCreateDate", datetime.now())),
        ...
        print(
            f"{Colors.OKGREEN}✅ Total de {len(users)} usuário(s) encontrado(s) no Cognito{Colors.ENDC}\n"
        )
        return users
...
def sync_user_to_couchdb(db, user: Dict, dry_run: bool = False) -> Dict:
    ...
    try:
        ...
        return {"success": False, "error": str(e), "user": user.get("email", "unknown")}
...
Exemplos:
  ...
  python scripts/sync_cognito_to_couchdb.py --type revenda      # Apenas revendas
    ...
    try:
        ...
        print(
            f"{Colors.WARNING}Certifique-se de que as credenciais AWS estão configuradas.{Colors.ENDC}"
        )
        sys.exit(1)
    ...
    for user in cognito_users:
        ...
        if result["success"]:
            ...
            print(
                f"{status_color}✅ {result['action'].upper()}: {result['user']} ({result['doc_id']}){Colors.ENDC}"
            )
        else:
            results["errors"].append(result)
            print(
                f"{Colors.FAIL}❌ ERRO: {result['user']} - {result['error']}{Colors.ENDC}"
            )
