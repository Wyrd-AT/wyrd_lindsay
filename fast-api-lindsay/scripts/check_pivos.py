...
def find_all_pivos(db) -> List[Dict]:
    ...
    try:
        print(f"{Colors.OKCYAN}1. Tentando query Mango (table='irrigadores')...{Colors.ENDC}")
        result = db.find({"selector": {"table": "irrigadores"}})
        pivos_mango = list(result)
        print(f"{Colors.OKGREEN}   ✅ Encontrados {len(pivos_mango)} pivô(s) com query Mango (table){Colors.ENDC}")
    ...
    try:
        ...
        print(f"{Colors.OKGREEN}   ✅ Encontrados {len(pivos_mango_type)} pivô(s) com query Mango (type){Colors.ENDC}")
        ...
        print(f"{Colors.WARNING}   Isso pode indicar que precisa criar um índice{Colors.ENDC}")
    ...
    try:
        ...
        all_docs = db.view('_all_docs', startkey='pivo:', endkey='pivo:\ufff0', include_docs=True)
        pivos_by_id = [
            row.doc for row in all_docs 
            if row.doc and (
                row.doc.get('table') == 'irrigadores' or 
                row.doc.get('type') == 'pivo'
            )
        ]
        print(f"{Colors.OKGREEN}   ✅ Encontrados {len(pivos_by_id)} pivô(s) por _id{Colors.ENDC}")
    ...
    try:
        ...
        print(f"{Colors.OKGREEN}   ✅ Encontrados {len(pivos_all)} pivô(s) no total{Colors.ENDC}")
...
def create_index_if_needed(db, create_index: bool = False):
    ...
    try:
        ...
        has_type_index = any(
            'type' in idx.get('fields', []) 
            for idx in indexes
        )
        
        if not has_type_index:
            print(f"\n{Colors.WARNING}⚠️  Não há índice para o campo 'type'{Colors.ENDC}")
            print(f"{Colors.WARNING}   Isso pode causar problemas na busca de pivôs{Colors.ENDC}")
...
def print_pivos(pivos: List[Dict]):
    ...
    if not pivos:
        print(f"{Colors.WARNING}⚠️  Nenhum pivô encontrado no banco de dados{Colors.ENDC}")
    ...
    for i, pivo in enumerate(pivos, 1):
        ...
        print(f"   {Colors.OKCYAN}Gerente:{Colors.ENDC} {pivo.get('gerente_id', 'N/A')}")
...
def analyze_pivos(pivos: List[Dict]):
    ...
    for field in required_fields:
        ...
        if missing > 0:
            print(f"  {Colors.WARNING}⚠️  Campo '{field}' faltando em {missing} pivô(s){Colors.ENDC}")
        else:
            print(f"  {Colors.OKGREEN}✅ Campo '{field}' presente em todos{Colors.ENDC}")
...
def main():
    ...
    parser.add_argument(
        "--create-index",
        action="store_true",
        help="Criar índice Mango se necessário"
    )
