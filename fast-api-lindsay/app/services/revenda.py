...
class RevendaService:
    ...
    def approve_revenda(self, revenda_id: str) -> Tuple[bool, str]:
        """
        Aprovar revenda (mudar status para 'active')

        Args:
            revenda_id: ID (UUID) da revenda ou email

        Returns:
            Tuple[bool, str]: (sucesso, mensagem)
        """
        # Se for UUID (tem hífens), buscar diretamente
        if "-" in revenda_id:
            revenda = self.get_revenda_by_uuid(revenda_id)
            if not revenda:
                return False, f"Revenda com ID '{revenda_id}' não encontrada"
        else:
            # Senão, procurar por email (pega o primeiro pending)
        ...
        try:
            ...
            revenda["approved_at"] = datetime.utcnow().isoformat()
            self.db.save(revenda)

            return True, f"Revenda '{revenda.get('name')}' aprovada"
        except Exception as e:
    ...
    def reject_revenda(
        ...
        try:
            ...
            revenda["rejected_at"] = datetime.utcnow().isoformat()

            if reason:
                revenda["rejection_reason"] = reason

            self.db.save(revenda)

            return True, f"Revenda '{revenda.get('name')}' rejeitada"
        except Exception as e:
            return False, f"Erro ao rejeitar revenda: {str(e)}"
